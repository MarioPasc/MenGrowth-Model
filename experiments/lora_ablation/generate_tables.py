...
def load_all_metrics(config: dict) -> Dict[str, Dict]:
    ...
    for cond in config["conditions"]:
        ...
        if men_dice_path.exists():
            ...
            metrics["dice_men_NCR"] = men_dice.get("dice_NCR")
            metrics["dice_men_ED"] = men_dice.get("dice_ED")
        ...
        if gli_dice_path.exists():
            ...
            metrics["dice_gli_NCR"] = gli_dice.get("dice_NCR")
            metrics["dice_gli_ED"] = gli_dice.get("dice_ED")
...
def generate_comprehensive_csv(
    ...
    - BraTS-MEN Dice: dice_men_mean, dice_men_NCR, dice_men_ED, dice_men_ET
    - BraTS-GLI Dice: dice_gli_mean, dice_gli_NCR, dice_gli_ED, dice_gli_ET
    ...
    column_order = [
        ...
        # BraTS-MEN Dice
        "dice_men_mean", "dice_men_NCR", "dice_men_ED", "dice_men_ET", "dice_men_std",
        # BraTS-GLI Dice
        "dice_gli_mean", "dice_gli_NCR", "dice_gli_ED", "dice_gli_ET", "dice_gli_std",
        # Linear probe R2
...
def generate_comprehensive_latex(
    ...
    |           | Mean NCR ED ET | Mean NCR ED ET | V L S mu  | V L S mu| Tot Trn|
    """
    lines = [
        ...
        r"Condition & Mean & NCR & ED & ET & Mean & NCR & ED & ET "
    ...
    for cond in config["conditions"]:
        ...
        men_ncr = fmt_dice(m.get("dice_men_NCR"))
        men_ed = fmt_dice(m.get("dice_men_ED"))
        men_et = fmt_dice(m.get("dice_men_ET"))

        gli_mean = fmt_dice(m.get("dice_gli_mean"))
        gli_ncr = fmt_dice(m.get("dice_gli_NCR"))
        gli_ed = fmt_dice(m.get("dice_gli_ED"))
        ...
        row = (
            f"{label} & "
            f"{men_mean} & {men_ncr} & {men_ed} & {men_et} & "
            f"{gli_mean} & {gli_ncr} & {gli_ed} & {gli_et} & "
