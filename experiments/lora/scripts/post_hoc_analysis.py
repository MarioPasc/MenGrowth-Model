...
Usage:
    python -m experiments.lora.scripts.post_hoc_analysis \
...
def load_predictions(condition_dir: Path) -> dict | None:
...
def load_metrics(condition_dir: Path) -> dict | None:
...
def generate_recommendation(comparisons: dict, metrics: dict) -> str:
    ...
    if significant_improvements:
        ...
        if abs(baseline_r2) > 1e-6:
            lines.append(
                f"• Improvement over baseline: {delta:.4f} ({delta / abs(baseline_r2) * 100:.1f}%)"
            )
        else:
...
def plot_r2_comparison(metrics: dict, output_dir: Path, title_suffix: str = "") -> None:
    ...
    plt.close()
    logger.info("Saved r2_comparison.{pdf,png}")
...
def plot_linear_vs_rbf(metrics: dict, output_dir: Path, title_suffix: str = "") -> None:
    ...
    for ax, feat in zip(axes, ["volume", "location", "shape"]):
        ...
        ax.bar(x + width / 2, rbf_r2, width, label="GP-RBF", color="darkorange", alpha=0.8)
    ...
    plt.suptitle(f"Linear vs GP-RBF Probe Performance{title_suffix}", y=1.02)
    plt.tight_layout()

    for ext in ["pdf", "png"]:
        plt.savefig(output_dir / f"linear_vs_rbf.{ext}")
    plt.close()
    logger.info("Saved linear_vs_rbf.{pdf,png}")
...
def plot_rank_vs_r2(metrics: dict, output_dir: Path, title_suffix: str = "") -> None:
    ...
    plt.close()
    logger.info("Saved rank_vs_r2.{pdf,png}")


def plot_semantic_vs_nosemantic(
    semantic_metrics: dict, nosemantic_metrics: dict, output_dir: Path
) -> None:
    ...
    plt.close()
    logger.info("Saved semantic_vs_nosemantic.{pdf,png}")
    ...
    for cond_dir in conditions_dir.iterdir():
        ...
        if log_path.exists():
            ...
            if df["train_loss"].max() > 100:
                anomalies.append(
                    f"WARNING [{cond}]: Very high training loss (max={df['train_loss'].max():.2f})"
                )
            ...
            if epochs < 50:
                anomalies.append(
                    f"WARNING [{cond}]: Only {epochs} epochs trained (early stopping?)"
                )
            ...
            if "val_dice_mean" in df.columns:
                ...
                if len(dice_values) > 10:
                    ...
                    if late_dice < early_dice - 0.05:
                        anomalies.append(
                            f"WARNING [{cond}]: Dice degraded during training "
        ...
        if metrics:
            ...
            for feat in ["volume", "location", "shape"]:
                ...
                # GP-RBF should generally be >= linear (captures nonlinear patterns)
                # Large negative gap suggests GP-RBF overfitting
                if gap < -0.2:
                    anomalies.append(
                        f"WARNING [{cond}]: GP-RBF underperforms linear for {feat} "
                        f"(gap={gap:.3f}) - possible GP-RBF overfitting"
                    )
            ...
            if var_mean < 0.01:
                anomalies.append(
                    f"WARNING [{cond}]: Very low feature variance ({var_mean:.6f}) "
        ...
        for fname in required_files:
            if not (cond_dir / fname).exists():
                ...
                alt_exists = any(
                    (cond_dir / f).exists()
                    for f in [
                        fname.replace(".pt", "_multi_scale.pt"),
...
def main(
    ...
    if "error" not in stats:
        ...
        logger.info("Saved statistical recommendation")
    ...
    if HAS_PLOTTING and stats.get("metrics"):
        ...
        plot_linear_vs_rbf(metrics, figures_dir, title_suffix)
    ...
    if compare_dir:
        ...
        if "metrics" in stats and "metrics" in stats2:
            if HAS_PLOTTING:
                plot_semantic_vs_nosemantic(stats["metrics"], stats2["metrics"], figures_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results-dir", type=str, required=True, help="Path to experiment results directory"
    )
    parser.add_argument(
        "--compare-dir", type=str, default=None, help="Optional: second experiment to compare"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
