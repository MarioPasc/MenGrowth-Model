#!/usr/bin/env python
# experiments/lora/analysis/statistical_analysis.py
...
Usage:
    python -m experiments.lora.analysis.statistical_analysis \
...
def generate_statistical_recommendation(
    ...
    for cond, comps in results.comparisons.items():
        ...
        for metric, comp in comps.items():
            if isinstance(comp, ConditionComparison):
                stars = (
                    "***"
                    if comp.test_result.p_value < 0.001
                    else "**"
                    if comp.test_result.p_value < 0.01
                    else "*"
                    if comp.test_result.p_value < 0.05
                    else "n.s."
                )
                lines.append(
    ...
    if not significant_improvements:
        ...
        lines.append(
            f"• Mean R² improvement: {best_delta:.4f} ({best_delta / baseline_r2 * 100:.1f}%)"
        )
        lines.append(
            f"• Effect size indicates {'meaningful' if abs(results.comparisons[best_cond].get('volume_neg_mse', ConditionComparison('', '', None, None, 0, None, StatisticalTest('', 0, 1, 0, '', False, False, 0, 0))).test_result.effect_size) > 0.5 else 'modest'} practical significance"
        )
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run statistical analysis for LoRA ablation")
    parser.add_argument(
        "--config",
        type=str,
