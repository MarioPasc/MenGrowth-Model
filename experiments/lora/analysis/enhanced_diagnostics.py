#!/usr/bin/env python
# experiments/lora/analysis/enhanced_diagnostics.py
...
Usage:
    python -m experiments.lora.analysis.enhanced_diagnostics \
...
def analyze_gradient_dynamics(
    ...
    if ratio.mean() < 0.2:
        issues.append(
            f"GRADIENT_IMBALANCE: Encoder receives only {ratio.mean():.1%} of decoder gradients"
        )
    elif ratio.mean() > 10:
        issues.append(
            f"GRADIENT_IMBALANCE: Encoder receives {ratio.mean():.1f}x decoder gradients (may be unstable)"
        )

    # Issue: High variance (unstable training)
    if results["encoder_cv"] > 2.0:
        issues.append(
            f"ENCODER_INSTABILITY: High gradient variance (CV={results['encoder_cv']:.2f})"
        )
...
def analyze_loss_dynamics(
    ...
    if "train_loss" in training_log.columns:
        ...
        results["train_loss_reduction"] = float(
            (train_loss[0] - train_loss[-1]) / (train_loss[0] + 1e-8)
        )
...
def analyze_feature_quality(
    ...
    if results["collapse_fraction"] > 0.5:
        issues.append(
            f"FEATURE_COLLAPSE: {results['collapse_fraction']:.1%} of dimensions have low variance"
        )

    if results["dim_utilization"] < 0.1:
        issues.append(
            f"LOW_UTILIZATION: Only {results['dim_utilization']:.1%} effective dimensionality"
        )
...
def analyze_probe_quality(
    ...
    if avg_evidence < -0.2:
        issues.append(
            f"RBF_UNDERFITTING: GP-RBF underperforms linear by {abs(avg_evidence):.2f} (may need more capacity)"
        )
    ...
    for cond in config["conditions"]:
        ...
        if log_path.exists():
            ...
            if grad_analysis.get("has_gradient_data"):
                logger.info(
                    f"  Gradients: enc={grad_analysis['encoder_grad_mean']:.4f}, "
        ...
        for level in ["multi_scale", "encoder10"]:
            ...
            if feat_path.exists():
                ...
                logger.info(
                    f"  Features ({level}): var={feat_analysis['variance_mean']:.4f}, "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run comprehensive diagnostics for LoRA ablation")
    parser.add_argument(
        "--config",
        type=str,
