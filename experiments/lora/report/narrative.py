...
def generate_introduction(exp: ExperimentData) -> SectionContent:
    ...
    section.paragraphs.append(
        ...
        "segmentation requires parameter-efficient adaptation of the encoder features."
    )

    if frozen is not None:
        frozen_men = frozen.dice_men.get("dice_mean", 0)
        section.paragraphs.append(
            f"The frozen model achieves Dice = {_fmt(frozen_men)} on BraTS-MEN."
        )

    section.paragraphs.append(
        f"This report evaluates {adapter_type} as a parameter-efficient method to "
        f"adapt the encoder, analyzing the trade-off between segmentation quality "
        f"(meningioma Dice) and feature quality (semantic probe R²)."
    )
...
def generate_rank_summary_section(exp: ExperimentData) -> SectionContent:
    ...
    for c in rank_conds:
        ...
        # Composite score: high MEN dice + high R²
        scores[c] = dice + 0.5 * max(r2, 0)

    recommended = max(scores, key=scores.get)  # type: ignore[arg-type]
    label = CONDITION_LABELS.get(recommended, recommended)

    section.paragraphs.append(
        f"Based on a composite score weighing segmentation quality and feature decodability, "
        f"the recommended configuration is "
        f"{label}."
    )
...
def generate_conclusion(exp: ExperimentData) -> SectionContent:
    ...
    section.paragraphs.append(
        f"{adapter_type} adaptation successfully improves meningioma segmentation. "
        f"The optimal configuration ({label}) achieves Dice = "
        f"{_fmt(best.dice_men.get('dice_mean', 0))} on meningioma. "
