#!/usr/bin/env python
# experiments/lora/engine/model_factory.py
...
Usage:
    from experiments.lora.engine.model_factory import create_ablation_model
...
from growth.models.encoder.swin_loader import load_full_swinunetr, load_swin_encoder
...
from growth.models.segmentation.seg_head import LoRASegmentationModel, SegmentationHead
...
def create_ablation_model(
    ...
    if decoder_type == "lightweight":
        ...
        raise ValueError(
            f"Unknown decoder_type: {decoder_type}. Must be 'lightweight' or 'original'."
        )
...
def _create_lightweight_model(
    ...
    if lora_rank is None:
        ...
        logger.info(
            f"Creating LoRA model with LIGHTWEIGHT decoder (rank={lora_rank}, alpha={lora_alpha})"
        )
