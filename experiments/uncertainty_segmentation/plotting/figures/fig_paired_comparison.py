...
def plot(
    ...
    x = merged[f"{dice_col}_bas"].values
    y = merged[f"{dice_col}_ens"].values
    ...
    ax_scatter.set_xlabel(f"Baseline Dice ({region_short})")
    ax_scatter.set_ylabel(f"Ensemble Dice ({region_short})")
    ...
    deltas = data.paired_differences[f"{dice_col}_delta"].values
    ...
    ax_hist.hist(deltas, bins=n_bins, color=C_FILL, alpha=0.5, edgecolor="white", lw=0.5)
    ...
    ax_hist.set_title(f"b) Paired differences ({region_short} Dice)", loc="left", fontweight="bold")
