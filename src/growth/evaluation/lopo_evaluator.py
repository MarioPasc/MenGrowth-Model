...
from growth.models.growth.base import (
    FitResult,
    GrowthModel,
    PatientTrajectory,
)
...
class LOPOEvaluator:
    ...
    def _run_fold(
        ...
        **model_kwargs,
        ...
        Args:
            model_class: GrowthModel subclass.
            train_patients: Training patients (held-out excluded).
            held_out: The held-out patient.
            **model_kwargs: Passed to model constructor.

        Returns:
            LOPOFoldResult for this fold.
        """
    ...
    def _apply_protocol(
        ...
        Args:
            model: Fitted growth model.
            patient: Held-out patient trajectory.
            protocol: Protocol name.

        Returns:
            List of prediction dicts, or None if the protocol is not applicable
            (e.g., ``all_from_first`` for patients with <3 timepoints).
        """
    ...
    def _protocol_last_from_rest(
        ...
        Works for any patient with >=2 timepoints.
        """
    ...
    def _protocol_all_from_first(
        ...
        Applicable only for patients with >=3 timepoints (need >=2 predictions
        to be informative).
        """
    ...
    def _compute_aggregate_metrics(self, fold_results: list[LOPOFoldResult]) -> dict[str, float]:
        ...
        Returns metrics in both log-space (the native space for log1p volumes)
        and original space (after expm1).
        """
        metrics: dict[str, float] = {}

        for protocol in self.prediction_protocols:
            # Collect all (predicted, actual) pairs for this protocol
            all_pred: list[float] = []
            ...
            metrics[f"{prefix}/r2_log"] = self._r2(actual_arr, pred_arr)
            metrics[f"{prefix}/mae_log"] = float(np.mean(np.abs(pred_arr - actual_arr)))
            metrics[f"{prefix}/rmse_log"] = float(np.sqrt(np.mean((pred_arr - actual_arr) ** 2)))
            ...
            metrics[f"{prefix}/r2_original"] = self._r2(actual_orig, pred_orig)
            metrics[f"{prefix}/mae_original"] = float(np.mean(np.abs(pred_orig - actual_orig)))
            metrics[f"{prefix}/rmse_original"] = float(
                np.sqrt(np.mean((pred_orig - actual_orig) ** 2))
            )

            # Calibration (fraction within 95% CI)
            within_ci = (actual_arr >= lower_arr) & (actual_arr <= upper_arr)
            metrics[f"{prefix}/calibration_95"] = float(np.mean(within_ci))

            # Mean CI width
            metrics[f"{prefix}/mean_ci_width_log"] = float(np.mean(all_ci_width))

        # Per-patient Pearson r (for patients with >=3 timepoints and
        # >=2 predictions under all_from_first)
