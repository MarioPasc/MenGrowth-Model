...
covariates from PatientTrajectory objects. Used by LME, H-GP, and ScalarGP
to ensure consistent covariate handling across all model types.
...
from .base import PatientTrajectory
...
def collect_covariates(
    ...
    for p in patients:
        ...
        cov_arr = np.array(
            [
                vals[name] if vals[name] is not None else means[name]
                for name in used_names
            ],
            dtype=np.float64,
        )
    ...
    if n_dropped > 0:
        logger.info(
            f"Dropped {n_dropped}/{len(patients)} patients due to missing covariates"
        )
    if n_imputed > 0:
        logger.info(
            f"Imputed covariates for {n_imputed}/{len(patients)} patients "
            f"(means: {means})"
        )
