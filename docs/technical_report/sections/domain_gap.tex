...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    $\text{MMD}^2$ (GLI vs.\ MEN) & \textit{[to be filled]} & $> 0$ confirms shift \\
    MMD $p$-value (permutation test) & \textit{[to be filled]} & $< 0.05$ confirms significance \\
    CKA (GLI vs.\ MEN) & \textit{[to be filled]} & Low $\Rightarrow$ different structure \\
    \midrule
    Linear probe $R^2$ (volume, MEN) & \textit{[to be filled]} & Baseline decodability \\
    Linear probe $R^2$ (location, MEN) & \textit{[to be filled]} & Baseline decodability \\
    Linear probe $R^2$ (shape, MEN) & \textit{[to be filled]} & Baseline decodability \\
    \bottomrule
  \end{tabular}
\end{table}
