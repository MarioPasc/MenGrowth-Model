...
This has three concrete benefits. First, generalization bounds from Bartlett et al., "Spectrally-normalized margin bounds for neural networks," *NeurIPS 2017*, scale with the product of spectral norms — fixing all to 1 yields tighter bounds. Gouk et al., "Regularisation of neural networks by enforcing Lipschitz continuity," *Machine Learning* 110:393–416, 2021, showed Lipschitz constraints are **particularly beneficial with limited training data**. Second, the contraction property prevents the MLP from creating artificial high-frequency structure in the latent space that could confuse downstream GP models. Third, for any downstream temporal model, well-conditioned initial conditions z(0) reduce prediction sensitivity to small perturbations — a 1-Lipschitz projection ensures that encoder feature noise does not amplify through the projection layer.
...
The GP-based temporal prediction approach similarly benefits from latent-space representations: Gaussian processes naturally handle arbitrary time gaps — precisely the irregular MRI intervals in the Andalusian cohort — while providing calibrated uncertainty quantification that is essential for clinical decision-making. The spectrally normalized SDP ensures well-conditioned latent representations, reducing sensitivity of GP predictions to small perturbations in the input features.
